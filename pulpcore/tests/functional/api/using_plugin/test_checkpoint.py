...
class TestCheckpointDistribution:
    ...
    def test_base_path_lists_checkpoints(self, setup, http_get):
    ...
    @pytest.mark.parallel
    def test_no_trailing_slash_is_redirected(self, setup, http_get):
        """Test checkpoint listing when path doesn't end with a slash."""

        pubs, distribution = setup

        response = http_get(distribution.base_url[:-1]).decode("utf-8")
        checkpoints_ts = set(re.findall(r"\d{8}T\d{6}Z", response))
    ...
    @pytest.mark.parallel
    def test_exact_timestamp_is_served(self, setup, http_get):
        pubs, distribution = setup

        pub_1_url = TestCheckpointDistribution.checkpoint_url(distribution, pubs[1].pulp_created)
        response = http_get(pub_1_url).decode("utf-8")

        assert f"<h1>Index of {urlparse(pub_1_url).path}</h1>" in response

    @pytest.mark.parallel
    def test_invalid_timestamp_returns_404(self, setup, http_get):
        _, distribution = setup
        with pytest.raises(ClientResponseError) as exc:
            http_get(f"{distribution.base_url}invalid_ts/")

        assert exc.value.status == 404

        with pytest.raises(ClientResponseError) as exc:
            http_get(f"{distribution.base_url}20259928T092752Z/")

        assert exc.value.status == 404

    @pytest.mark.parallel
    def test_checkpoint_artifact_is_served(self, setup, http_get):
        pubs, distribution = setup
        pub_1_url = TestCheckpointDistribution.checkpoint_url(distribution, pubs[1].pulp_created)
        pub_2_url = TestCheckpointDistribution.checkpoint_url(distribution, pubs[2].pulp_created)
        ...
        assert artifact_names[0] == "0"
        assert artifact_names[1] == "1"

    @pytest.mark.parallel
    def test_non_checkpoint_timestamp_is_redirected(self, setup, http_get):
        pubs, distribution = setup
        # Using a non-checkpoint publication timestamp
        pub_3_url = TestCheckpointDistribution.checkpoint_url(distribution, pubs[3].pulp_created)
        pub_4_url = TestCheckpointDistribution.checkpoint_url(distribution, pubs[4].pulp_created)
    ...
    @pytest.mark.parallel
    def test_arbitrary_timestamp_is_redirected(self, setup, http_get):
        pubs, distribution = setup
        pub_1_url = TestCheckpointDistribution.checkpoint_url(distribution, pubs[1].pulp_created)
        arbitrary_url = TestCheckpointDistribution.checkpoint_url(
            distribution, pubs[1].pulp_created + timedelta(seconds=1)
        )
    ...
    @pytest.mark.parallel
    def test_current_timestamp_serves_latest_checkpoint(self, setup, http_get):
        pubs, distribution = setup
        pub_3_url = TestCheckpointDistribution.checkpoint_url(distribution, pubs[3].pulp_created)
        now_url = TestCheckpointDistribution.checkpoint_url(distribution, datetime.now())
    ...
    @pytest.mark.parallel
    def test_before_first_timestamp_returns_404(self, setup, http_get):
        pubs, distribution = setup
        pub_0_url = TestCheckpointDistribution.checkpoint_url(distribution, pubs[0].pulp_created)
    ...
    @pytest.mark.parallel
    def test_future_timestamp_returns_404(self, setup, http_get):
        _, distribution = setup
        url = TestCheckpointDistribution.checkpoint_url(
            distribution, datetime.now() + timedelta(days=1)
        )
    ...
    @staticmethod
    def checkpoint_url(distribution, timestamp):
        return f"{distribution.base_url}{Handler._format_checkpoint_timestamp(timestamp)}/"
